...
class Oploverz : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val episodes = document.select("a.ring-offset-background.gap-2").mapIndexedNotNull { index, element ->
            val episode = element.select("p:first-child").text().filter { it.isDigit() }.toIntOrNull()
                ?: (index + 1)
